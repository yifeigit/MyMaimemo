...
async function aiStory(settings, words, { highlight } = {}) {
  ...
  const hlList = (highlight && highlight.length ? highlight : words)
    ...
    .join(" ");
  const prompt =
    `你是一位英语写作教练。请用给定单词创作一组相互独立的英文句子（类似高质量例句集），不必构成连贯故事，每个句子各自独立成句。\n` +
    `\n` +
    `【最重要的一条】你的输出必须 ONLY 是句子正文的 <line> 内容，除此之外什么都不要输出：\n` +
    `- 不要任何开场白、寒暄、解释，例如"好的""以下是我创作的句子""我来写"等一律禁止；\n` +
    `- 不要任何思考过程、内心独白、推理说明；\n` +
    ...
    `\n` +
    `一、句子要求：\n` +
    `1. 每个句子独立完整、自然地道，表达一个清晰的意思；句子之间不需要情节连贯，可分别取材于日常、学习、工作、旅行等不同场景，读起来像母语者写的例句。\n` +
    `2. 句子数量与单词数量相当（至少每词一句），每个句子至少用上其中一个给定单词，尽量让所有单词都出现，不堆砌单词、不为了用词而造病句。\n` +
    `3. 必须覆盖所有单词：${wordList}\n` +
    `4. 背诵重点词：${hlList}。这些词在英文句中出现时用 **双星号** 包裹（如 He showed great **courage**.），其余英文单词不包裹；对应的中文翻译里，表示这些词的对应中文词也要用 **双星号** 包裹（如 “他展现了巨大的**勇气**.”），其余中文词不包裹。\n` +
    `\n` +
    `二、输出格式（每个句子一对）：\n` +
